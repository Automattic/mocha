...
Runner.prototype.fail = function (test, err) {
  ...
  ++this.failures;
  if (test.parent) {
    ++test.parent.failures; // suite.failures
  }
  test.state = 'failed';
...
Runner.prototype.runTests = function (suite, fn) {
  ...
  function next (err, errSuite) {
    ...
    if ((self.failures && suite._bail) || (suite.failures && suite._bailSuite)) {
      return fn();
    }
...
Runner.prototype.runSuite = function (suite, fn) {
  ...
  if (!total || ((self.failures && suite._bail) || (suite.failures && suite._bailSuite))) {
    return fn();
  }
